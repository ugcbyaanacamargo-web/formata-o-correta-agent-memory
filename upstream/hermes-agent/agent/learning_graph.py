    ...
    for source, root in skill_roots:
        for skill_md in root.rglob("SKILL.md") if root.exists() else ():
            ...
            nodes[name] = SkillNode(
                name=name,
                category=str(cat) if cat else parts[-3] if len(parts) >= 3 else "general",
                source=source,
...
def density_stats(nodes: dict[str, SkillNode], edges: list[tuple[str, str]]) -> dict[str, Any]:
    ...
    cats = Counter(x.category for x in nodes.values())
    n = len(nodes) or 1
    return {
        "nodes": len(nodes),
...
def build_learning_graph() -> dict[str, Any]:
    roots = [
        ...
        ("profile", get_hermes_home() / "skills"),
    ]
    learned_skills = {
        name: node
        for name, node in build_skill_nodes(roots).items()
